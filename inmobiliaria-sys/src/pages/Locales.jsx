...
export default function Locales() {
  ...
  useEffect(() => {
    const fetchLocales = async () => {
      const q = query(collection(db, "locales"), orderBy("precio", "desc"))
  ...
  const totalPaginas = Math.ceil(locales.length / porPagina)
  const inicio = (pagina - 1) * porPagina
  const visibles = locales.slice(inicio, inicio + porPagina)
  ...
  return (
    ...
    <div className="container py-5">
      <h1 className="text-bebas tamano2 text-center">Locales</h1>
          ...
          <div key={c.id} className="col-md-4 mb-4">
            <div className="card h-100">
              ...
              <div className="card-body d-flex flex-column">
                ...
                <Link to={`/propiedad/locales/${c.id}`} className="btn bg-dark text-white mb-2">
                  Ver propiedad
                </Link>
              </div>
            </div>
